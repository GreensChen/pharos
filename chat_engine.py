...
如果 user 暗示要結束這輪對話（例如打「好先這樣吧」、「夠了」、「差不多」、「結束」、
「先收」、「OK 就這樣」、「謝謝」之類的收尾語），你**這次的回覆**必須照下面格式：
...
## 我給你的觀察
（3-5 個 bullet，整理你給 user 的判斷 / 挑戰 / 引用過的關鍵 [[書名]]）

## 待釐清 / 可繼續挖
（1-3 個 bullet，這次沒講完、user 之後可以追問的方向）

如果 user 還在繼續討論（提新問題、延伸思考、表達好奇），就照常回答、**不要寫 __END__**。

判斷時保守一點 — 寧可多聊一輪、不要太早 END。除非 user 訊號明確（短訊息 + 收尾語）。
"""
...
def parse_end_signal(answer: str) -> tuple[bool, str]:
    """偵測 Gemini 回應是否以 __END__ 開頭。
    回傳 (is_end, summary_text)。is_end=True 時 summary_text 是去掉 __END__ 的摘要。"""
    stripped = answer.lstrip()
    if stripped.startswith("__END__"):
        # 拿掉第一行 __END__
        rest = stripped[len("__END__"):].lstrip("\n").strip()
        return True, rest
    return False, ""
